...
public class StringHasher implements Iterable<ArrayList<Find>>{
	...
	public void Ingest(String s,String name) {
		...
		for(int i=1;i<=s.length()-length;i++) {
			...
			/*how we actually calculate the hash
			 *given a hash for a:b we subtract a*(numChars^(length-1)) we multiply by numChars and we add b+1
			 *this effectively gives the hash of (a+1):(b+1) in constant time 
			 *
