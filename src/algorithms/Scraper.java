	...
	private static void DownloadGenome(String capture, String genome,String name) {
		...
		while(links.find()) {
			try {
				...
				if(cdss.find()) {
					String file=getFile("https://www.ncbi.nlm.nih.gov/sviewer/viewer.cgi?tool=portal&save=file&log$=seqview&db=nuccore&report=genbank&id="+cdss.group(1)+"&extrafeat=976&conwithfeat=on&withparts=on" );
					...
					File f=new File("Viruses/"+name.replaceAll("<.*?>", "").replaceAll("[\\\\/]", "|")+"-"+sequence+".gb");
					sequence++;
